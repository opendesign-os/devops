...
Plane 以 Dokploy 的 **Compose 服务**接入，编排文件在部署仓库，变量填在 Dokploy UI。通用的机器准备见 [devops/README.md](devops/README.md)，本文只写 Plane 特有部分。

| 项 | 位置 |
|---|---|
| 编排文件 | 部署仓库 `projects/plane/docker-compose.yaml` |
| 变量清单 | 部署仓库 `env/plane.env.example`，实际填在 Dokploy 服务的 Environment |
| 运行机器 | 机器 B |
...
| 镜像来源 | 全部经 Harbor 的 `dockerhub` 代理缓存项目，需先在 Harbor 建好该项目 |
...
Create Service → **Compose**，Provider 选 Git 指向部署仓库，Compose 路径填 `projects/plane/docker-compose.yaml`，Server 选**机器 B**。

### 步骤 2：填环境变量

在服务的 Environment 编辑器里粘贴 `env/plane.env.example` 的内容后逐项修改。Dokploy 会把它写成 `.env` 放在 compose 同目录，供 `${VAR}` 插值。
...
点 Deploy。首次拉取约 5 GB 镜像，`migrator` 迁移完成后 `api` 才启动，整体 3~10 分钟。在 Logs 里观察，`migrator` 退出码 0 属正常。
...
数据备份在机器 B 上执行（Plane 的数据在 docker 命名卷里，Dokploy 不备份业务数据）：
...
容器名前缀由 Dokploy 生成，用 `docker ps` 确认实际名称后再执行。
