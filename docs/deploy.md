...
全文以本仓库已克隆到两台服务器的 `/srv/devops` 为基准：手册在机器上直接读，机器 A 的 Zot 也在检出目录里启动，不往服务器传文件、也不留第二份配置。克隆是第二节第 1 步。
...
克隆到 `/srv/devops`，后面每一节都以它为前提。克隆完手册也在机器上了，`less /srv/devops/docs/deploy.md` 直接读。
...
配置与状态分两处：检出里的 [compose.yaml](../registry/compose.yaml) 与 [config.json](../registry/config.json) 由容器就地读，服务器上不留第二份；`/srv/registry/` 只放运行时状态，即 `htpasswd` 与镜像层 `data/`。目录划分见 [layout.md](layout.md) 第三节，配置项含义、权限矩阵、保留策略调参与运维命令见 [registry.md](registry.md)。
