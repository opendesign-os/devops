...
两台机器只能走公网，仓库因此跑在隧道内，HTTP 明文只存在于隧道内部，`insecure-registries` 的豁免也仅限隧道地址。**不要把 5000 开到公网** —— `docker login` 凭据是 base64 明文，截获即可推镜像投毒。

不加防火墙规则，而是让容器**只绑定 `10.8.0.1:5000`**，端口从一开始就没监听在公网网卡上。代价是 wg0 必须先于 Docker 起来。
...
UI 由 `extensions.ui` 提供，与 API 同端口，`admin` 账号登录，访问命令见 [deploy.md](deploy.md) 第八节。

它依赖 `extensions.search` 的 CVE 能力，漏洞库定期下载、额外占内存与磁盘。机器 A 吃紧时把 `search` 与 `ui` 两段一起删掉，镜像存取与代理缓存不受影响。
...
| `docker pull` 超时、`data/docker/` 不生成 | 机器 A 到上游不通，换掉该规则的 `urls` 主地址，挑法见第七节 |
...
`extensions.sync.registries` 是**有序数组**，每条规则 = 一个上游地址 + 管哪些仓库。当前两条：
...
拆两条是因为 daocloud 回源不到 `makeplane/*`，而 `library/*`、`valkey/*` 正常。`**` 匹配一切，所以 makeplane 两条都命中、先试第 1 条失败再落第 2 条，自带降级。**新增规则必须插在 `**` 那条之前**，放后面永远轮不到。

`content` 三个字段决定本地仓库名怎么映射回上游：

| 字段 | 含义 |
|---|---|
| `prefix` | 匹配**上游**仓库名，`**` 跨层级通配 |
| `destination` | 存到本地哪个前缀下，两条都是 `/docker` |
| `stripPrefix` | 是否砍掉 `prefix` 里的字面量部分 |

`prefix: "**"` 没有字面量部分，`stripPrefix` 填什么都一样；`prefix: "makeplane/**"` 的字面量是 `makeplane/`，要保留它才能拼出 `docker/makeplane/plane-proxy`，**必须 `false`**。填反了本地变成 `docker/plane-proxy`，Plane 的 compose 就对不上。

### 一条规则只配一个 URL

多写几个 `urls` 不是逐镜像回退，而且有实打实的代价：

**它不解决问题** —— Zot 靠 `/v2/` 探活挑一个能连的用。daocloud 正是 `/v2/` 好好的、就是不给 `makeplane/*`，永远不会切换。

**它拖慢每一次拉取** —— 镜像已在缓存里时 Zot 仍会回上游校验一次（日志 `sync: syncing image` → `skipping image because it's already synced`）。曾给第 2 条加过 `https://index.docker.io` 兜底，而机器 A 连不上 Docker Hub：

| 第 2 条的 urls | 缓存命中后耗时 |
|---|---|
| `[daocloud, index.docker.io]` | **30.1 秒**（正好一个 `retryDelay`） |
| `[daocloud]` | **1.0~1.6 秒** |

10 个镜像的栈白等 2 分钟，叠加到并发部署上足以把本来能过的拉取推回 EOF。

### 挑上游

候选站挨个测，`makeplane/plane-proxy` 是 daocloud 出不了的那个，`library/alpine` 作对照：

```bash
for m in docker.m.daocloud.io docker.1ms.run docker.xuanyuan.me docker.1panel.live hub.rat.dev index.docker.io; do printf '%-24s ' "$m"; printf 'makeplane=%-5s' "$(curl -s -m 15 -o /dev/null -w '%{http_code}' https://$m/v2/makeplane/plane-proxy/manifests/v1.4.1)"; printf 'library=%s\n' "$(curl -s -m 15 -o /dev/null -w '%{http_code}' https://$m/v2/library/alpine/manifests/3.20)"; done
```

| 返回 | 含义 |
|---|---|
| `200` | 匿名直出，可用 |
| `401` | 要 token，Zot 会自己换，也算可用；但**不代表回源一定成功**，daocloud 就是 401 之后才 403 |
| `403` | 上游明确拒绝这个仓库 |
| `429` | 在限流，不可靠 |
| `000` | 机器 A 连不上 |

只有 `200`/`401` 值得试，改完必须实拉一个镜像验证，光看状态码不够。

### 首次拉取报 EOF 是正常的

排障时不知道这条会白跑很久。

`onDemand` 回源是**同步阻塞**的：本地没有该镜像时 Zot 挂住请求，先把整个镜像同步下来再回应。且同步 manifest index 下的**全部条目** —— `plane-proxy:v1.4.1` 落了 `linux/amd64`、`linux/arm64` 和两个 attestation，机器 A 只用得上第一个。

等待远超 docker 客户端的忍耐度，客户端先断开报 `EOF`。**但这不代表回源失败**：实测 `plane-proxy` 客户端 1m48s 报 EOF，同一时刻 Zot 日志是 `successfully synced image` + `statusCode=200 latency=1m47s`，重拉一次 5.5 秒完成。

**判断回源通没通看 Zot 日志，不看 docker 报错**：

| Zot 日志 | 含义 | 处理 |
|---|---|---|
| `successfully synced image` | 通了，客户端等不及而已 | 重拉一次 |
| `failed to sync image` + `unauthorized access` | 上游不给这个仓库 | 换上游 |
| 什么都没有 | 请求没到 Zot | 查隧道、查 `insecure-registries` |

### 换上游之后：带重试预热

改 `config.json` → push → 机器 A `sudo git -C /srv/devops pull` → `cd /srv/devops/registry && sudo docker compose restart`，然后**串行预热且必须带重试**，只跑一遍会被首次 EOF 全部绊倒：
...
10 个镜像约 20~40 分钟。看进度：

```bash
cat /root/prewarm.log; du -sh /srv/registry/data/docker
```
...
换第三方镜像站等于把回源信任交出去。`preserveDigest: true` 只保证内容与 manifest digest 自洽，**不校验这个 manifest 是否就是 Docker Hub 上那个**。要紧的镜像从 Docker Hub 页面抄下官方 digest 比对：
