...
检出里的文件不手工改 —— 改动一律回本仓库 push 再 pull。服务落在哪台机器由 Dokploy 服务的 **Server** 字段决定；下面各节按当前分布写：自研应用在机器 B，Plane 在机器 A。
...
代价：反馈慢（要构建到那一步才知道测试挂）、拦不住 PR（webhook 是 push 触发）、做不了轻量检查（只跑 lint 也得走完整构建）、测试输出夹在构建日志里。
...
最后一条是硬边界：**镜像的产地必须唯一**。各推各的会让 `apps/<app>` 有两个来源，回滚时分不清 tag 是谁推的，保留策略计数也会乱。要换就整体换，不能并存。
