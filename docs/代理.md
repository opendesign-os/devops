...
```bash
env | grep -i proxy; git config --global --get-regexp proxy; grep -rn 7898 /etc/dnf/dnf.conf /etc/systemd/system/docker.service.d/ ~/.bashrc 2>/dev/null; ss -lnt | grep 7898
```
